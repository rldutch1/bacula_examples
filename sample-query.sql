...
SELECT DISTINCT Job.JobId as JobId, convert(Client.Name using utf8mb4) as Client,
 ...
 AND JobStatus IN ('T','W') AND Job.JobId=File.JobId
 AND Path.PathId=File.PathId
 AND File.FileName!='Rxxxyyyzzz123456789H' /*Filename does not match, so everything is pulled.*/
 AND File.FileIndex > 0
 AND Job.JobId = 23 /*Enter the JobId that you want to see.*/
...
SELECT DISTINCT Job.JobId as JobId, convert(Client.Name using utf8mb4) as Client,
 ...
 AND JobStatus IN ('T','W') AND Job.JobId=File.JobId
 AND Path.PathId=File.PathId
 AND convert(File.FileName using utf8mb4) = 'known_hosts.old'
 AND File.FileIndex > 0
 AND Job.JobId = 16
 ORDER BY Job.StartTime;
...
SELECT DISTINCT Job.JobId as JobId, convert(Client.Name using utf8mb4) as Client,
 ...
 AND JobStatus IN ('T','W') AND Job.JobId=File.JobId
 AND Path.PathId=File.PathId
 AND convert(File.FileName using utf8mb4) = 'TheFileName'
 AND File.FileIndex > 0
 -- AND Job.JobId = 16
 ORDER BY Job.StartTime;
